...
class TestApiCall(Base):
    ...
    def setup_test(self, global_config: GlobalConfiguration):
        ...
        self.http_response_catcher: HttpCallBack = (self.global_config.http_client_configuration.http_callback or
                                                    HttpResponseCatcher())
        self.http_client: HttpClient = (self.global_config.http_client_configuration.http_client or
                                        MockHttpClient())
    ...
    def test_end_to_end_with_uninitialized_http_callback(self):
        ...
        assert self.http_client.should_retry is False # type: ignore[attr-defined]
        assert self.http_client.contains_binary_response is False # type: ignore[attr-defined]
        assert self.http_response_catcher.response is None # type: ignore[attr-defined]
    ...
    def test_end_to_end_without_endpoint_configurations(self):
        ...
        assert self.http_client.should_retry is False # type: ignore[attr-defined]
        assert self.http_client.contains_binary_response is False # type: ignore[attr-defined]
        assert self.http_response_catcher.response.status_code == 200 # type: ignore[attr-defined]
        assert ApiHelper.json_serialize(Base.employee_model()) == ApiHelper.json_serialize(actual_employee_model)
    ...
    def test_end_to_end_with_endpoint_configurations(self, input_to_retry, input_contains_binary_response,
        ...
        assert self.http_client.should_retry == expected_to_retry # type: ignore[attr-defined]
        assert self.http_client.contains_binary_response == expected_contains_binary_response # type: ignore[attr-defined]
        assert self.http_response_catcher.response.status_code == 200 # type: ignore[attr-defined]
        assert ApiHelper.json_serialize(Base.employee_model()) == ApiHelper.json_serialize(actual_employee_model)
