...
class TestAuthHelper:
    ...
    def test_token_expiry_with_skew_time(self):
        current_utc_timestamp = AuthHelper.get_current_utc_timestamp()
        actual_token_expiry_value = AuthHelper.get_token_expiry(current_utc_timestamp, 5, 2)
        expected_token_expiry_value = current_utc_timestamp + int(3)
        assert actual_token_expiry_value == expected_token_expiry_value
